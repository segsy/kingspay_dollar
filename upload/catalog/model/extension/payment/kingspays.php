...
class ModelExtensionPaymentKingspays extends Model {
    ...
    public function getMethod($address, $total) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_kingspays_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('payment_kingspays_total') > 0 && $this->config->get('payments_kingspays_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_kingspays_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                ...
                'terms' => '',
                'sort_order' => $this->config->get('payment_kingspays_sort_order')
            );
        }
    ...
    public function getCards($customer_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "kingspay_card WHERE customer_id = '" . (int)$customer_id . "'");
    ...
    public function addCard($order_id, $card_data) {
        $this->db->query("INSERT into " . DB_PREFIX . "kingspay_card SET customer_id = '" . $this->db->escape($card_data['customer_id']) . "', order_id = '" . $this->db->escape($order_id) . "', digits = '" . $this->db->escape($card_data['Last4Digits']) . "', expiry = '" . $this->db->escape($card_data['ExpiryDate']) . "', type = '" . $this->db->escape($card_data['CardType']) . "', token = '" . $this->db->escape($card_data['Token']) . "'");
    }

    public function deleteCard($token) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "kingspay_card WHERE customer_id = '" . $this->customer->isLogged() . "' AND token = '" . $this->db->escape($token) . "'");
    ...
    public function addOrder($order_info, $order_code) {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "kingspay_order` SET `order_id` = '" . (int)$order_info['order_id'] . "', `order_code` = '" . $this->db->escape($order_code) . "', `date_added` = now(), `date_modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
    ...
    public function getOrder($order_id) {
        ...
        if ($qry->num_rows) {
            $order = $qry->row;
            $order['transactions'] = $this->getTransactions($order['kingspay_order_id']);
    ...
    public function addTransaction($kingspay_order_id, $type, $order_info) {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "kingspay_order_transaction` SET `kingspay_order_id` = '" . (int)$kingspay_order_id . "', `date_added` = now(), `type` = '" . $this->db->escape($type) . "', `amount` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
    }

    public function getTransactions($kingspay_order_id) {
        $qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "kingspay_order_transaction` WHERE `kingspay_order_id` = '" . (int)$kingspay_order_id . "'");
    ...
    public function recurringPayment($item, $order_id_rand, $token) {
        ...
        $this->model_extension_payment_kingspays->logger($order);

        $response_data = $this->model_extension_payment_kingspays->sendCurl('orders', $order);

        $this->model_extension_payment_kingspays->logger($response_data);
    ...
    public function cronPayment() {
        ...
        $log = new Log('kingspay_recurring_orders.log');
    ...
    public function getKingspayOrder($kingspay_order_id) {
        $qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "kingspay_order WHERE order_code = " . (int)$kingspay_order_id);
        return $qry->row;
    }

    public function updateCronJobRunTime() {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'kingspays' AND `key` = 'payment_kingspays_last_cron_job_run'");
        $this->db->query("INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'kingspays', 'payment_kingspays_last_cron_job_run', NOW(), 0)");
    }

    public function sendCurl($url, $order = null,$amount,$currency_code,$payment,$device_signature,$email,$ip_address) {
        $curl = curl_init();
    ...
    public function logger($data) {
        if ($this->config->get('payment_kingspays_debug')) {
            $log = new Log('payment_kingspays_debug.log');
